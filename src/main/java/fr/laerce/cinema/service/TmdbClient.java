...
public class TmdbClient {
    ...
    private long secondsBeforeReset(String value){
        ...
        LocalDateTime resetTime = LocalDateTime.ofInstant(Instant.ofEpochSecond(timestamp), ZoneId.systemDefault());
    ...
    public void getMovieByTmdbID(long id) throws Exception{
        ...
        String ressourceUrl = "https://api.themoviedb.org/3/movie/" + id + "?api_key=" + apiKey + "&language=fr-FR";
        ...
        System.out.println("--------\nRequête restantes" + stripBraces(response.getHeaders().get("x-ratelimit-remaining").toString()));
        reset =secondsBeforeReset(response.getHeaders().get("x-ratelimit-reset").toString());
        System.out.println("Temps restant avant le reset : " + reset + "\n\n");
